...
=pod
	    push @{$qc->{amr_genes}}, [$feature->{id}, "$gene_name", $feature->{function}, $classification];
			print "$feature->{id}\t$gene_name\t$classification\n";

			if (ref($qc->{amr_gene_summary}->{$classification}) eq "ARRAY" && $gene_name)
			{
	    	@{$qc->{amr_gene_summary}->{$classification}} = @{$qc->{amr_gene_summary}->{$classification}}, $gene_name;
			}else
			{
				push @{$qc->{amr_gene_summary}->{$classification}}, $gene_name;
			}
=cut 
