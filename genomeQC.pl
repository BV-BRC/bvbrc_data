...
foreach my $genome_id (@genome_ids){
	...
	getGenomeStats($genome_id, $genome);
	getSequenceStats($genome_id, $genome);
	getFeatureStats($genome_id, $genome);
	#getSpeciesStats($genome) if $genome->{species};

	if (ref $genome->{genome_quality_flags} ne "ARRAY" ){
		$genome->{genome_quality} = "High";
	}elsif (scalar @{$genome->{genome_quality_flags}} <= 2){
		$genome->{genome_quality} = "Medium" unless join(",", @{$genome->{genome_quality_flags}})=~/Inconsist|No CDS|No tRNA|No rRNA/;
		$genome->{genome_quality} = "Low" if join(",", @{$genome->{genome_quality_flags}})=~/Inconsist|No CDS|No tRNA|No rRNA/;
	}else{
		$genome->{genome_quality} = "Low";
	}
...
sub getGenomeStats {
	...
	my ($genome_id, $genome) = @_;

	my $core = "/genome";
	my $query = "/select?q=genome_id:$genome_id";
	my $fields = "&fl=genome_name,genus,species,genome_status,genome_length,sequences,patric_cds";
	my $rows = "&rows=1";
	my $sort = "";
	...
	chomp $result;
	my ($genome_name,$genus,$species,$genome_status,$genome_length,$sequences,$patric_cds) = split /\t/, $result;
	...
	$genome->{cds_ratio} = $genome->{patric_cds}*1000/$genome->{genome_length};
	...
	push @{$genome->{genome_quality_flags}}, "Too many contigs" if $genome->{sequences} > 1000;
...
sub getSequenceStats {
	...
	push @{$genome->{genome_quality_flags}}, "Inconsistent data: number of contigs" unless $contigs == $genome->{sequences};
...
sub getFeatureStats {
	...
	my $query = "/select?q=genome_id:$genome_id AND annotation:PATRIC";
	...
	$genome->{cds_ratio} = $genome->{cds} * 1000 / $genome->{genome_length};
	$genome->{hypothetical_cds_ratio} = $genome->{hypothetical_cds} / $genome->{cds};
	$genome->{partial_cds_ratio} = $genome->{partial_cds} / $genome->{cds};
	$genome->{plfam_cds_ratio} = $genome->{plfam_cds} / $genome->{cds};
